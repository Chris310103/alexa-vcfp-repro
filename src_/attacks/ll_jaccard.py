...
class lljaccardModel(AttackModel):
...
if __name__=="__main__":
    ...
    path=pp.parent/"external"/"trace_csv"
    traces=loader_all_trace(path)
    data=stratified_split_by_label(traces)
    train_data=data[0]
    test_data=data[1]
    a=lljaccardModel(rounding=100)
    train=a.fit(train_data)
    rel=a.predict_one(test_data[10])
    print ("true label:", test_data[10].label)
    print ("pre:",rel)
